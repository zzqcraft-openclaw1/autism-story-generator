...
The canonical output contract is defined in `docs/story-output-json-schema.md`.

For MVP, each generated story should contain at least these top-level fields:
...
- `topic` and `target_skill` belong inside `story_request`, not as conflicting top-level fields
- child-facing story text belongs inside the nested `story` object, not a flat `story_text` field
- the intended lesson belongs in `caregiver_note.intended_lesson`
- review hints should use `review_flags`

Within the nested `story` object, MVP should include:
- `summary`
- `tone_tags[]`
- `opening`
- `sections[]`
